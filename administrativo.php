...
$igreja = $_SESSION['usuarioIgreja'];
 	...
 	//Consultas SQL 
include "sql/admin/sql_administrativo.php";
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
      ...
      <tbody>
        <?php while($linhas_aniversario_batismo = mysqli_fetch_assoc($resultado_aniversario_batismo)){ ?>
          <tr>
            ...
            <td><center><?php echo $linhas_aniversario_batismo['classe']; ?></center></td>
...
$sqlclasse = mysqli_query($conn , "SELECT *, Count(id_classes) QTD FROM classes WHERE igreja=$igreja");
          while ($dadosclasse = mysqli_fetch_array($sqlclasse)){
            extract($dadosclasse);
          }
          echo "$QTD";

          $vetor['QTD'] =  mysqli_query($conn , "SELECT *, Count(id_classes) QTD FROM classes WHERE igreja=$igreja");


         for ($i=1; $i <= $QTD; $i++) { 
          

          $sqlpontos = mysqli_query($conn , "SELECT *, Sum(pontuacao) ponto FROM pontuacao WHERE classe=$numerodaclasse AND  igreja=$igreja");
          while ($dadospontos = mysqli_fetch_array($sqlpontos)){
            extract($dadospontos);
          }
          echo "<tr>";
                  echo "<td><center>". $numerodaclasse ."</center></td>";//</center>";
