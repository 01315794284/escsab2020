      ...
      <form class="form-signin" method="POST" action="valida.php" autocomplete="off">
        ...
        <select class="form-control" name="igreja">
             <!-- <option>Selecione a Igreja</option>-->
                <?php
                $result_igreja = "SELECT id_igreja, nome_igreja FROM igreja";
                $resultado_igreja2 = mysqli_query($conn, $result_igreja);
                while($linhas_igreja = mysqli_fetch_assoc($resultado_igreja2)){ ?>
                  <option value="<?php echo $linhas_igreja['id_igreja']; ?>"><?php echo $linhas_igreja['nome_igreja']; ?>
                 <?php
                }
                ?>
              </select>
